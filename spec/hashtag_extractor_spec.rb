RSpec.describe Hshtg::Stream::HashtagExtractor, '#hashtag_extractor', focus: true do
  context 'hashtag parser' do
    ...
    it 'can group by hash tags without case sensitivity' do
      text = '#one #one #two # three #One'
      tags = Hshtg::Stream::HashtagExtractor.hash_tags_from_tweet(text)
      top  = Hshtg::Stream::HashtagExtractor.extract_tags!(tags, 5, false)
    ...
    it 'can group by hash tags with case sensitivity' do
      text = '#one #one #two # three #One'
      tags = Hshtg::Stream::HashtagExtractor.hash_tags_from_tweet(text)
      top  = Hshtg::Stream::HashtagExtractor.extract_tags!(tags, 5, true)
      expect(top.first.tag).to eq('one')
    ...
    it 'can deal with finding no hash tags' do
      tags = Hshtg::Stream::HashtagExtractor.hash_tags_from_tweet('')
    ...
    it 'can deal with finding nil text' do
      tags = Hshtg::Stream::HashtagExtractor.hash_tags_from_tweet(nil)
      expect(tags.count).to be(0)
    end

    it 'can deal with a nil list' do
      top = Hshtg::Stream::HashtagExtractor.extract_tags!(nil, nil, nil)
      expect(top.count).to eq(0)
    end
  end
end
